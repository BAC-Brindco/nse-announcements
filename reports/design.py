...
Kept byte-identical to the copy in the nse-surveillance-pipeline (deals) repo.
The two pipelines are separate repositories, so this is a vendored copy rather
than a shared import; when one changes, port the change to the other or the
house style forks. Every BAC report renders through this module so the style is
shared rather than copied. A colour or size literal in a report's own source is a bug: table amber
